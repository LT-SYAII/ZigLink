...
pub fn main() !void {
    ...
    defer server.deinit();
    std.debug.print("⚡ ZigLink Server jalan di http:
    while (true) {
        ...
        const is_protected = std.mem.eql(u8, path, "/") or std.mem.eql(u8, path, "/info");
        const has_cookie = std.mem.indexOf(u8, request_text, "zig_guard=passed") != null;
        if (is_protected and !has_cookie) {
            try serveFile(connection.stream, "src/challenge.html");
            continue;
        }
...
fn loadDB() !void {
    ...
    std.debug.print("Loaded {d} links.\n", .{db.count()});
}
fn saveDB() !void {
...
fn serveJsonStats(stream: net.Stream) !void {
    ...
    while (db_iter.next()) |entry| {
        ...
        first = false;
        const item = try std.fmt.allocPrint(allocator, "{{\"code\":\"{s}\",\"url\":\"{s}\",\"created\":{d}}}", .{entry.key_ptr.*, entry.value_ptr.url, entry.value_ptr.timestamp});
        defer allocator.free(item);
        try links_json.appendSlice(item);
    }
    try links_json.appendSlice("]");
    const count = db.count();
    const info = sysinfo.collect(allocator) catch sysinfo.SystemInfo{ .distro="Unknown", .kernel="-", .arch="-", .hostname="-", .cpu_model="-", .cpu_cores=0, .ram_total_mb=0, .ram_free_mb=0, .uptime_sec=0, .user="-" };
    const response_body = try std.fmt.allocPrint(allocator, "{{\"status\":\"online\",\"os\":{{\"distro\":\"{s}\",\"kernel\":\"{s}\",\"arch\":\"{s}\",\"hostname\":\"{s}\",\"cpu\":\"{s}\",\"cores\":{d},\"ram_total_mb\":{d},\"ram_free_mb\":{d},\"uptime_sec\":{d},\"user\":\"{s}\"}},\"total_links\":{d},\"data\":{s}}}", .{info.distro, info.kernel, info.arch, info.hostname, info.cpu_model, info.cpu_cores, info.ram_total_mb, info.ram_free_mb, info.uptime_sec, info.user, count, links_json.items});
...
fn handleShorten(stream: net.Stream, body: []const u8) !void {
    ...
    if (std.mem.indexOf(u8, url, " ") != null or (!std.mem.startsWith(u8, url, "http:
    ...
    const new_data = LinkData{ .url = try allocator.dupe(u8, url), .timestamp = std.time.timestamp() };
    try db.put(code, new_data);
    saveDB() catch |err| { std.debug.print("Error saving DB: {any}\n", .{err}); };
